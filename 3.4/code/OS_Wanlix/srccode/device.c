...
/***********************************************************************************
函数功能: 向串口打印任务字符串, 向LCD屏打印任务切换图形.
入口参数: uiTask: 任务切换图形的任务编号.
返 回 值: none.
***********************************************************************************/
void DEV_TaskPrintMsg(U32 uiTask)
{
    /* 向串口打印数据 */
    printf("\r\nTask%d is running! Time is: %d s", uiTask, DEV_GetTaskTime());

    /* 向LCD屏打印任务切换图形 */
    DEV_DrawTaskSwitchGraph(uiTask);
}

/***********************************************************************************
函数功能: 增加任务运行的时间.
入口参数: uiTime: 任务增加的运行时间.
返 回 值: none.
***********************************************************************************/
void DEV_AddTaskTime(U32 uiTime)
{
    guiTimer += uiTime;
}
...
返 回 值: 任务已运行的时间.
***********************************************************************************/
U32 DEV_GetTaskTime(void)
{
    return guiTimer / 1000;
}
...
void DEV_TaskSwitchDisplayInit(void)
{
    ...
    {
        DEV_LcdDrawPic(TSDSPXSTART + (TSDSPYWID / 2) + (TSDSPTASKITV * (i + 1)) - 8,
...
返 回 值: none.
***********************************************************************************/
void DEV_DrawTaskSwitchGraph(U32 uiTask)
{
    /* 存储任务切换信息 */
    DEV_SaveTaskSwitchMsg(uiTask);
...
返 回 值: none.
***********************************************************************************/
void DEV_SaveTaskSwitchMsg(U32 uiTask)
{
    U32 uiPreTail;

    /* 存储任务切换信息 */
    gstrTsMsgStr.astrMsg[gstrTsMsgStr.uiTail].uiTask = uiTask;
    gstrTsMsgStr.astrMsg[gstrTsMsgStr.uiTail].uiTolalTime = DEV_GetTaskTime();
...
void DEV_PrintTaskSwitchMsgToLcd(void)
{
    ...
    uiHead = gstrTsMsgStr.uiHead;
    uiTail = gstrTsMsgStr.uiTail;

    /* 环形缓冲区尾指向最后存储的任务信息位置 */
    uiTail = (uiTail + TASKSWITCHMSGLEN - 1) % TASKSWITCHMSGLEN;

    /* 计算需要打印的图形长度 */
    uiDisplayLen = (gstrTsMsgStr.astrMsg[uiTail].uiTolalTime
                    - gstrTsMsgStr.astrMsg[uiHead].uiTolalTime) * TSDSPSWTITV;
    ...
    if(uiDisplayLen >= TSDSPTIMENUM * TSDSPTIMEITV)
    {
        ...
        /* 更新显示时间 */
        DEV_PrintTaskSwitchTime(gstrTsMsgStr.astrMsg[uiTail].uiTolalTime);
    }
    ...
    {
        /* 计算一个任务可打印的长度 */
        if(uiDisplayLen >= gstrTsMsgStr.astrMsg[uiTail].uiRunTime * TSDSPSWTITV)
        {
            uiLeftDisplayLen = gstrTsMsgStr.astrMsg[uiTail].uiRunTime * TSDSPSWTITV;
        }
        else
        {
        ...
        /* 打印任务运行的直线 */
        DEV_LcdDrawYLine(TSDSPXSTART + gstrTsMsgStr.astrMsg[uiTail].uiTask * TSDSPTASKITV,
        ...
        if(0 != uiDisplayLen)
        {
            uiNextTask = gstrTsMsgStr.astrMsg[uiTail].uiTask;

            /* 指向前一个任务 */
            uiTail = (uiTail + TASKSWITCHMSGLEN - 1) % TASKSWITCHMSGLEN;

            /* 根据任务在LCD屏的位置画2个任务之间切换的连线 */
            if(gstrTsMsgStr.astrMsg[uiTail].uiTask < uiNextTask)
            {
                /* 打印2个任务之间切换的连线 */
                DEV_LcdDrawXLine(TSDSPXSTART + gstrTsMsgStr.astrMsg[uiTail].uiTask * TSDSPTASKITV,
                                 TSDSPYSTART + uiDisplayLen,
                                 (uiNextTask - gstrTsMsgStr.astrMsg[uiTail].uiTask) * TSDSPTASKITV,
                                 TSDSPLINEWID);
            }
            else
            {
                ...
                DEV_LcdDrawXLine(TSDSPXSTART + uiNextTask * TSDSPTASKITV,
                                 TSDSPYSTART + uiDisplayLen,
                                 (gstrTsMsgStr.astrMsg[uiTail].uiTask - uiNextTask) * TSDSPTASKITV,
                                 TSDSPLINEWID);
            }
        }
    }
...
void DEV_PrintTaskSwitchTime(U32 uiLastTime)
{
    ...
    for(i = 0; i <= TSDSPTIMENUM; i++)
    {
        /* 计算显示时间的位数 */
        digits_num = 0;
        time = uiLastTime - (TSDSPTIMENUM - i) * TSDSPBTNUM;
