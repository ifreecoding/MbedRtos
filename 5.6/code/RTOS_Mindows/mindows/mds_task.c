...
U32 MDS_TaskDelete(M_TCB* pstrTcb)
{
    ...
    /* 将任务从任务链表删除 */
    (void)MDS_DlistCurNodeDelete(&gstrTaskList, &pstrTcb->strTaskQue.strQueHead);
           ...
           U8* pucTaskStack, U32 uiStackSize, U8 ucTaskPrio, M_TASKOPT* pstrTaskOpt)
{
    ...
    /* 将新建任务加入到任务链表 */
    MDS_DlistNodeAdd(&gstrTaskList, &pstrTcb->strTaskQue.strQueHead);

#ifdef MDS_CPUSHARE

    /* 初始化任务的CPU占有率 */
    MDS_TaskCpuShareInit(pstrTcb);
...
返 回 值: 任务链表根节点指针.
***********************************************************************************/
M_DLIST* MDS_GetTaskLinkRoot(void)
{
    return &gstrTaskList;
}
