...
void MDS_RootTask(void)
{
    ...
    pstrTcb = MDS_TaskCreate("Test3", TEST_TestTask3, NULL, gaucTask3Stack,
                             TASKSTACK, 2, &strOption);
    ...
    (void)MDS_TaskDelay(8000);

    /* 系统运行80秒后删除task3任务 */
    (void)MDS_TaskDelete(pstrTcb);

    (void)MDS_TaskDelay(2000);

    /* 系统运行100秒后删除root任务 */
    (void)MDS_TaskDelete(MDS_GetCurrentTcb());
}
