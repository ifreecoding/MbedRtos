...
U32 guiCpuSharePeriod;                  /* CPU占有率统计周期计数 */
...
函数功能: 检查任务栈剩余未用的长度.
入口参数: pstrTcb: 需要检查的任务的TCB.
返 回 值: 任务栈剩余未用的长度, 单位: 字节.
***********************************************************************************/
U32 MDS_TaskStackCheck(M_TCB* pstrTcb)
{
    U32 uiStackBottom;
    U32 i;
...
/***********************************************************************************
函数功能: 初始化任务的CPU占有率结构.
入口参数: pstrTcb: 需要初始化的任务TCB.
返 回 值: none.
***********************************************************************************/
void MDS_TaskCpuShareInit(M_TCB* pstrTcb)
{
...
返 回 值: none.
***********************************************************************************/
void MDS_CpuShareStatistic(M_TCB* pstrOldTcb, M_TCB* pstrNewTcb)
{
    ...
    /* 当前运行任务没有被删除, 统计CPU占有率 */
    if(NULL != pstrOldTcb)
    {
        /* CPU占有率计数, tick定时器是24bits递减定时器 */
        if(suiTick == guiTick) /* 计数器未溢出 */
        {
    ...
    if(CPUSHARETIME == guiCpuSharePeriod)
    {
        ...
        /* 循环统计每个任务的CPU占有率 */
        while(NULL !=
              (pstrTaskList = MDS_DlistNextNodeEmpInq(&gstrTaskList, pstrTaskList)))
        {
            /* 指向任务的TCB */
            pstrTaskTcb = ((M_TCBQUE*)pstrTaskList)->pstrTcb;

            /* 计算CPU占有率百分比, 采用四舍五入计算 */
...
/***********************************************************************************
函数功能: 获取任务的CPU占有率.
入口参数: pstrTcb: 需要获取CPU占有率的任务TCB指针.
返 回 值: 任务的CPU占有率, 单位: 1%.
***********************************************************************************/
U32 MDS_GetCpuShare(M_TCB* pstrTcb)
{
