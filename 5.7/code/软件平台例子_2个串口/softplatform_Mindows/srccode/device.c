...
/***********************************************************************************
函数功能: 在LCD屏上显示字符串.
入口参数: pstring: 需要显示的字符串的指针.
          len: 字符串的长度.
          x: x轴显示的起始坐标.
          y: y轴显示的起始坐标.
返 回 值: none.
***********************************************************************************/
void display_string(U8* pstring, U32 len, U32 x, U32 y)
{
    U32 i;
    U16 pen_color;

    /* 获取画笔颜色 */
    pen_color = LCDGETPENCOLOR;
    ...
    /* 设置画笔颜色 */
    LCDSETPENCOLOR(pen_color);

    /* 显示字符串中的每个字符 */
    for(i = 0; i < len; i++)
    {
        DEV_LcdDrawPic(x, y + (i * 8), 16, 8, DEV_CalcInvertFontAddr(pstring[i]));
    }
}

/***********************************************************************************
函数功能: 设置画笔颜色.
入口参数: color: 需要设置的画笔颜色.
返 回 值: none.
***********************************************************************************/
void set_pen_color(U16 color)
{
    LCDSETPENCOLOR(color);
}
...
{
    /* 无效字符 */
    if(!((' ' == ucAscii) || ((ucAscii >= '0') && (ucAscii <= '9'))
         || ((ucAscii >= 'A') && (ucAscii <= 'Z'))
         || ((ucAscii >= 'a') && (ucAscii <= 'z'))))
    {
        return NULL;
...
U8* DEV_CalcInvertFontAddr(U8 ucAscii)
{
    ...
    if(' ' == ucAscii)
    {
        return &gaucInvertFont[(16 * (10 + 26 + 26))];
    }
    /* 0~9之间的字符 */
    else if(ucAscii <= '9')
    {
